...
int main(int argc, const char * const *argv) {
	//msdf -font C:\Windows\Fonts\arialbd.ttf 'M' -o msdf.png -size 32 32 -pxrange 4 -autoframe -testrender render.png 1024 1024
	//////////////////////////////////
    ...
    if (orientation == REVERSE) {
		...
		/*for (int y = msdf.height() - 1; y >= 0; --y)
			for (int x = 0; x < msdf.width(); ++x) {
				cout << setw(10) << msdf(x, y).r << setw(10) << msdf(x, y).g << setw(10) << msdf(x, y).b << endl;
			}*/
    }
    ...
    switch (mode) {
        ...
        case MULTI:
            ...
            if (testRender) {
				...
				/*for (int y = render.height() - 1; y >= 0; --y)
					for (int x = 0; x < render.width(); ++x) {
						cout << setw(10) << render(x, y) << setw(10) << render(x, y) << setw(10) << render(x, y) << endl;
					}*/
				...
				显示图片测试
				*/
				cv::Mat image = cv::imread("render.png", 1);
...
//					cout << setw(4) << clamp(int(msdf(x, y).r * 0x100), 0xff) << setw(4) << clamp(int(msdf(x, y).g * 0x100), 0xff) << setw(4) << clamp(int(msdf(x, y).b * 0x100), 0xff) << endl;
//				}
//		}
//		destroyFont(font);
//	}
